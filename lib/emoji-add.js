...
class EmojiAdd {
  ...
  async upload(srcList) {
    let emojiList, aliasList;

    console.log(`Attempting to upload ${srcList.length} emoji to ${this.subdomain}`);

    [aliasList, emojiList] = _.partition(srcList, 'is_alias');
    ...
    return {subdomain: this.subdomain, emojiList: srcList, errorList: errors};
  }
  ...
  static async createMultipart(emoji, token) {
    if (emoji.is_alias === 1) {
      ...
      let emojiData = await FileUtils.getData(emoji.url || emoji.src);
      return {
        'token': token,
        'name': emoji.name,
        'mode': 'data',
