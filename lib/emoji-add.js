...
class EmojiAdd {
  ...
  async upload(src) {
    let masterList; let emojiList; let
      aliasList;
    ...
    const emojiResultArray = await Promise.all(emojiList.map(emoji => this.uploadSingle(emoji)));
    const aliasResultArray = await Promise.all(aliasList.map(emoji => this.uploadSingle(emoji)));
    const resultArray = emojiResultArray.concat(aliasResultArray);

    const errors = _.filter(resultArray);
    const errorJson = _.groupBy(errors, 'error');
