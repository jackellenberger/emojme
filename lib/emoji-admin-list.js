...
class EmojiAdminList {
  ...
  async get(bustCache) {
    ...
    if (bustCache || FileUtils.isExpired(path)) {
      ...
      const fullList = [].concat.apply([], emojiLists);
  ...
  async getAdminListPages() {
    const body = await this.slack.request(this.endpoint, this.createMultipart(1));
    if (!body.ok) {
    ...
    const pages = body.paging.pages;
    const promiseArray = [Promise.resolve(body.emoji)];
  ...
  static diff(srcLists, srcSubdomains, dstLists, dstSubdomains) {
    ...
    _.zipWith(dstSubdomains, dstLists, (dstSubdomain, dstEmojiList) => {
      ...
      diffs.push({ dstSubdomain, srcSubdomains: _.without(srcSubdomains, dstSubdomains), emojiList: missingEmojiList });
    });
  ...
  static async save(emojiList, subdomain, users) {
    ...
    users.forEach((user) => {
      ...
      const dirPath = `build/${subdomain}/${user}`;
