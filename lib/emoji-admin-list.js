...
class EmojiAdminList {
  constructor(args) {
    ...
    this.token = Array.isArray(args.token) ? args.token[0] : args.token;
    this.subdomain = Array.isArray(args.subdomain) ? args.subdomain[0] : args.subdomain;
  ...
  static diff(emojiLists, subdomains) {
    let diffsToUpload = {};
    let annotatedEmojiLists = _.zipWith(emojiLists, subdomains, (emojiList, subdomain) => {
      return _.map(emojiList, (emoji) => {
        return _.extend({}, emoji, {original_subdomain: subdomain});
      });
    });

    let masterEmojiList = _.unionBy(...annotatedEmojiLists, 'name');
    subdomains.forEach((subdomain) => {
      let missingEmojiList = _.filter(masterEmojiList, (obj) => {
        return obj.original_subdomain != subdomain;
      });
      ...
      diffsToUpload[subdomain] = missingEmojiList;
    });
