...
const duration = require('./since-duration');
...
class EmojiAdminList {
  constructor(subdomain, token, output, since) {
  ...
  async get(options = {}) {
    var since, adminList = null;
    const fullAdminList = await this.getFullAdminList(options.bustCache);

    if(options.since) {
      try {
        since = duration.before(duration.parse(options.since)).getTime();
        adminList = _.filter(fullAdminList, ({created}) => created >= since);
      } catch(e) {
        logger.warning(`unable to parse argument "since: ${options.since}", please use ISO-8601 Duration format. Defaulting to unlimited emoji age.`);
      }
    }

    return adminList || fullAdminList;
  }

  async getFullAdminList(bustCache) {
    const path = `./build/${this.subdomain}.adminList.json`;

    if (bustCache || FileUtils.isExpired(path)) {
      const emojiLists = await this.downloadAdminList();
